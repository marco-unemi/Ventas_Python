...
class Sale(Icalculo):
    ...
    def add_detail(self, prod, qty):
        ...
        self.discount = self.cal_discount(
            self.subtotal, self.percentage_discount)
    ...
    def print_invoice(self, company):
        ...
        print(blue_color +
              f"Empresa: {company.business_name} Ruc: {company.ruc}", end='')
    ...
    def getJson(self):
        ...
        invoice = {"factura": self.invoice, "Fecha": self.date.strftime("%Y-%m-%d"), "cliente": self.client.fullName(
        ), "subtotal": self.subtotal, "descuento": self.discount, "iva": self.iva, "total": self.total, "detalle": []}
        for det in self.sale_detail:
